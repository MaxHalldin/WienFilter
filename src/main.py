...
with op1, op2, inp:
    ...
    gs = GridSearch(input=inp, measuring_time=0.5, parent=win.main_widget(), move_knobs=True, measurement_str=None)
    volt_values = [round(x) for x in np.arange(0, 301, 30)]
    mamp_values = [round(x) for x in np.arange(0, 1001, 100)]
    gs.add_device(com1, volt_values, 0.1)
    gs.add_device(com2, mamp_values, 0.1)
